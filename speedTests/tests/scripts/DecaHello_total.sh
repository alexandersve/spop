...
do
    ...
    (time $(bash ${KETTLE_DIR}kitchen.sh -file=${KETTLE_DIR}../input/DecaHello.kjb &> /dev/null)) &>> ${LOCAL_TARGET_DIR}kettle.log
    (time $(bash ${HOP_DIR}hop-run.sh -j default -r local -f ${SOURCE_DIR}hop/input/DecaHello.hwf &> /dev/null)) &>> ${LOCAL_TARGET_DIR}hop.log
done
