...
do
    ...
    (time $(bash ${KETTLE_DIR}kitchen.sh -file=${KETTLE_DIR}../input/CentaHello.kjb &> /dev/null)) &>> ${LOCAL_TARGET_DIR}kettle.log
    (time $(bash ${HOP_DIR}hop-run.sh -j default -r local -f ${SOURCE_DIR}hop/input/CentaHello.hwf &> /dev/null)) &>> ${LOCAL_TARGET_DIR}hop.log
done
